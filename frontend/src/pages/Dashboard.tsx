  ...
  return (
    <div className="p-6 max-w-7xl mx-auto text-white">
      ...
      <div className="mb-6 p-4 bg-gray-800 rounded-lg flex flex-wrap gap-4 items-center">
        ...
        <FilterButton label="Spoofable" field="spoofable" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {data?.pages.map((group, i) =>
            group.map((server, index) => {
              const isLastElement =
                data.pages.length === i + 1 && group.length === index + 1;
              ...
              return (
                ...
                >
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <span
                      className={`w-3 h-3 rounded-full ${server.was_online ? "bg-green-500" : "bg-red-500"}`}
                    ></span>
                  </div>
                  <div
                    className="prose prose-invert prose-sm max-w-none bg-gray-900 p-2 rounded mb-2"
                    dangerouslySetInnerHTML={{
                      __html: server.description_html,
                    }}
                  />
