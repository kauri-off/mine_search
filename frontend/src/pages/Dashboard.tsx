...
export const Dashboard = () => {
  ...
  return (
    <div className="flex flex-1 overflow-hidden">
      ...
      <div
        className={`fixed inset-0 z-40 bg-black/60 md:hidden transition-opacity duration-200 ${showMobileFilters ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}
        onClick={() => setShowMobileFilters(false)}
      />
      <div
        className={`fixed inset-y-0 left-0 z-50 md:hidden transition-transform duration-200 ease-in-out ${showMobileFilters ? "translate-x-0" : "-translate-x-full"}`}
      >
        <FilterSidebar
          {...filterSidebarProps}
